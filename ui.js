...
function drawPerson(errorsCount) {
    ...
    canvasContext.fillRect(5, 220, 80, 10);
    canvasContext.fillRect(7, 10, 5, 210);
...
function drawBoard(lettersState) {
    ...
    for (let i = 0; i < lettersState.length; i++) {
        ...
        const yPosition = 35 * Math.floor((i / lettersInLine) + 1) + 80;//испр на 80
...
function drawAnswerState(answerState) {
    const xPositionH = 100;
    const yPositionH = 30;
    canvasContext.clearRect(xPositionH, yPositionH - 30, canvas.width - xPositionH, 40);
    canvasContext.font = "28px serif";
    canvasContext.fillText('Угадайте слово из ' + answerState.length + ' букв', xPositionH, yPositionH);

    const xPosition = 150;//испр 100
    const yPosition = 80;//испр 40
